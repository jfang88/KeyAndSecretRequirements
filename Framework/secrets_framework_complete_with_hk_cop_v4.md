...
A **secret** is any sensitive value that must be protected from unauthorized disclosure because possession or knowledge of that value can grant access, prove identity, or enable privileged actions. Typical secrets include passwords, API tokens, client secrets, bearer tokens, and some private-key material when exported or handled outside protected hardware.

A **key** is a specific cryptographic value used by an algorithm for encryption, decryption, signing, verification, key wrapping, or message integrity operations. All keys are sensitive, but not all secrets are cryptographic keys.

In enterprise use, the distinction is important:

- A **secret** is the broader category.
- A **cryptographic key** is a specialized type of secret used for cryptographic operations.
- A **credential** is a secret or key used to authenticate an identity.
...
13. NIST SP 800-63 Digital Identity Guidelines for identity proofing, authentication, federation, and authenticator lifecycle management.
14. Hong Kong OCCICS Code of Practice (Version 1.0, 1 January 2026) for the Protection of Critical Infrastructures (Computer Systems) Ordinance.
...
The document also contains 22 clickable external HTTPS links in the reference sections and appendix reference table.
