...
- Physical cash handling cryptography outside enterprise IT scope.
...
### 2.1 Control families most used
...
| Key establishment and management | NIST SC-12, SC-13 |
...
A **secret** is any value that grants access, proves identity, protects confidentiality or integrity, or enables cryptographic operations.

A **key** is a cryptographic parameter used for encryption, decryption, signing, verification, or integrity operations.
...
6. OWASP *Key Management Cheat Sheet*.
7. OWASP *Secrets Management Cheat Sheet*.
8. OWASP *Logging Cheat Sheet*.
9. OWASP *CI/CD Security Cheat Sheet*.
10. PCI DSS v4.0 Requirement 3 and related cryptographic key management guidance.
11. ISO/IEC 27001:2022 aligned guidance relevant to cryptography, access control, logging, and operations.
12. FIPS 140-3 for cryptographic module validation.
...
### 20.4 Appendices recommended for local version

The enterprise version of this document should add the following appendices:

- Approved algorithms and key lengths.
- Exception register and compensating control template.
- Standard Vault path taxonomy.
- Rotation runbooks by platform.
- Detection rule catalogue.
- Break-glass exercise checklist.
- Inventory field dictionary and JSON schema.
- Approved CI/CD runner hardening baseline.
...
| Non-Huawei cloud, hybrid, or on-prem zones | CyberArk Conjur / CyberArk vault services | Service secrets, machine credentials, CI/CD secrets, non-Huawei runtime secrets | Sidecar, init container, shared file volume, or controlled Kubernetes Secret sync |
...
2. Non-Huawei workloads should use CyberArk Conjur as the default enterprise vault for application and machine secrets outside Huawei infrastructure. [page:3][web:94]
...
- Huawei Cloud CSMS and DEW documentation for secret creation, secret versioning, KMS-backed encryption, and CCE Secrets Manager for DEW integration. [web:80][web:81][web:82][web:83][web:93][page:2]
- CyberArk Secrets Provider for Kubernetes documentation for sidecar, namespace service, shared file, and Kubernetes Secret delivery patterns. [web:85][page:3][web:94]
- Kubernetes official good-practice guidance for Secret encryption, RBAC minimization, namespace isolation, and external secret providers. [web:92][page:4]
